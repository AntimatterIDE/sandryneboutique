...
export interface TiltedTilesProps {
  ...
  images?: string[];
  /** Product tiles with optional links (takes precedence over `images`) */
  tiles?: TiltedTile[];
...
const tileClassName =
  "w-full shrink-0 bg-neutral-200 bg-cover bg-center bg-no-repeat dark:bg-neutral-800";
          ...
          if (tile.href) {
            return (
              <Link
                ...
                href={tile.href}
                aria-label={tile.alt ? `Shop ${tile.alt}` : "View product"}
                className={`${tileClassName} relative block cursor-pointer`}
                style={style}
              />
            );
          }
...
const TiltedTiles: React.FC<TiltedTilesProps> = ({
  ...
  const pool = useMemo<TiltedTile[]>(() => {
    if (tiles?.length) return tiles;
    const sources = images.length ? images : DEFAULT_IMAGES;
